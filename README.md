...
- 2.Man10ResourcePack環境に採用テクスチャを導入する
- 3.必ずマイクラ内で適用作業を行い、バグやミッシングがないことを確認すること
- 4.２回以上確認したのち、GithubDesktop側でsummaryに追加テクスチャを記入  (例: AK47,M4A1実装 iron_hoe編集)
- 5.もう１度確認した後、Pushボタンを押してMasterに実装　その後テクスチャ配信または再起動で実装されます。
